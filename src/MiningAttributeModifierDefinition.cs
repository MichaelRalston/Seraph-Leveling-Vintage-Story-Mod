...
{
    public record class MiningAttributeModifierDefinition : LeveledToolAttributeModifierDefinition
    {
        public override int CalculateBonus(EntityPlayer entity, LeveledToolAttributeModifierProgressData progress)
        {
            return Math.Min(progress.TotalCredits, GlobalMaxCredits);
        }

        public override int ApplyBonus(IServerPlayer player, LeveledToolAttributeModifierProgressData progressData)
        {
