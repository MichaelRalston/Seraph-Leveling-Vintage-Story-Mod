...
namespace SeraphLeveling
{
    public class SeraphLevelingModSystem : ModSystem
    {
        ...
        private void ApplyServerHarmonyPatches(ICoreServerAPI api)
        {
            ...
            try
            {
                if (!Harmony.HasAnyPatches(HARMONY_SERVER_ID))
                {
                    ...
                    // Patch Collectible.DamageItem for various tool durability traits.
                    PatchItemDamage(api);
        ...
        private void PatchSmithingCrafting(ICoreServerAPI api)
        {
            try
            {
                ...
                var checkIfFinishedMethod = AccessTools.Method(anvilType, "CheckIfFinished");
                if (checkIfFinishedMethod == null)
                {
                    api.Logger.Debug("[SeraphLeveling] Could not find BlockEntityAnvil.CheckIfFinished method for crafting hooks");
                    return;
                }

                // Get our prefix method
                var prefixMethod = AccessTools.Method(typeof(CraftingPatches), nameof(CraftingPatches.BlockEntityAnvil_CheckIfFinished_Prefix));
                if (prefixMethod == null)
                {
                    api.Logger.Error("[SeraphLeveling] Could not find BlockEntityAnvil_CheckIfFinished_Prefix method!");
                    return;
                }

                // Get our postfix method
                var postfixMethod = AccessTools.Method(typeof(CraftingPatches), nameof(CraftingPatches.BlockEntityAnvil_CheckIfFinished_Postfix));
                if (postfixMethod == null)
                {
                    api.Logger.Error("[SeraphLeveling] Could not find BlockEntityAnvil_CheckIfFinished_Postfix method!");
                    return;
                }

                serverHarmony.Patch(checkIfFinishedMethod, prefix: new HarmonyMethod(prefixMethod), postfix: new HarmonyMethod(postfixMethod));
                api.Logger.Notification("[SeraphLeveling] Successfully patched BlockEntityAnvil.CheckIfFinished for crafting hooks");
            }
            catch (Exception ex)
            {
        ...
        private void PatchItemDamage(ICoreServerAPI api)
        {
            try
            {
                var collectableType = AccessTools.TypeByName("Vintagestory.API.Common.CollectibleObject");
                if (collectableType == null)
                {
                    api.Logger.Warning("[SeraphLeveling] Could not find EntityBehaviorHarvestable type");
                    return;
                }
                var damageItemMethod = AccessTools.Method(collectableType, "DamageItem");
                if (damageItemMethod == null)
                {
                    api.Logger.Warning("[SeraphLeveling] Could not find DamageItem method in CollectibleObject");
                    return;
                }
                var prefixMethod = AccessTools.Method(typeof(ItemDamagePatches), nameof(ItemDamagePatches.DamageItem_Prefix));
                serverHarmony.Patch(damageItemMethod, prefix: new HarmonyMethod(prefixMethod));
                api.Logger.Notification("[SeraphLeveling] Successfully patched CollectibleObject.DamageItem for durability traits");
            }
            catch (Exception ex)
            {
                api.Logger.Warning($"[SeraphLeveling] Failed to patch DamageItem: {ex.Message}");
            }
        }
